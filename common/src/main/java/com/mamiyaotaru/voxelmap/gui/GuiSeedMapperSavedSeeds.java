...
public class GuiSeedMapperSavedSeeds extends GuiScreenMinimap {
    ...
    public GuiSeedMapperSavedSeeds(Screen parent) {
    ...
    public void init() {
        ...
        addButton = addRenderableWidget(new Button.Builder(Component.literal("Add"), button ->
                this.minecraft.gui.setScreen(new GuiSeedMapperEditSavedSeed(this, null, null)))
                .bounds(this.width / 2 - 155, buttonY, buttonWidth, 20).build());
        editButton = addRenderableWidget(new Button.Builder(Component.literal("Edit"), button -> editSelected())
                .bounds(this.width / 2 - 79, buttonY, buttonWidth, 20).build());
        removeButton = addRenderableWidget(new Button.Builder(Component.literal("Remove"), button -> removeSelected())
                .bounds(this.width / 2 - 3, buttonY, buttonWidth, 20).build());
        addRenderableWidget(new Button.Builder(Component.translatable("gui.done"), button -> onClose())
                .bounds(this.width / 2 + 73, buttonY, buttonWidth, 20).build());
        reloadEntries();
    }

    public void reloadEntries() {
        entries.clear();
        entries.addAll(settings.getSavedSeedsSnapshot().entrySet());
    ...
    private void editSelected() {
        ...
        this.minecraft.gui.setScreen(new GuiSeedMapperEditSavedSeed(this, entry.getKey(), entry.getValue()));
    }
    ...
    private void removeSelected() {
        ...
        settings.putSavedSeed(entries.get(selectedIndex).getKey(), "");
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        graphics.centeredText(this.getFont(), Component.literal("Saved Seeds"), this.width / 2, 20, 0xFFFFFFFF);
