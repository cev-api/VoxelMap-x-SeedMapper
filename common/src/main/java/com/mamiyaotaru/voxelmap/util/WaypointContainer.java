...
public class WaypointContainer {
    ...
    private void renderSign(SubmitPass pass, Matrix4fStack matrixStack, Camera camera, Waypoint waypoint, TextureAtlas textureAtlas, boolean isPointedAt, boolean isHighlighted, double distance, double baseX, double baseY, double baseZ) {
        ...
        if (isPointedAt && options.waypointSignLayout != 0 && (!hideLabels || customLayout)) {
            ...
            if (options.waypointSignLayout == 4) {
                // Preserve the fork's independent name/distance placement and existing configuration.
                mainLabel = options.waypointNamesLocation == 0 ? "" : waypoint.name;
                if (isHighlighted && waypointManager.isCoordinateHighlight(waypoint) && options.waypointNamesLocation != 0) {
                    mainLabel = "X:" + waypoint.getXInCurrentDimension() + ", Y:" + waypoint.getY() + ", Z:" + waypoint.getZInCurrentDimension();
                }
                subLabel = options.waypointDistancesLocation == 0 ? "" : distanceLabel;
                moveLabelDown = options.waypointNamesLocation == 2;
                if (!subLabel.isEmpty()) {
                    if (mainLabel.isEmpty()) {
                        moveLabelDown = options.waypointDistancesLocation == 2;
                        mainLabel = subLabel;
                        subLabel = "";
                ...
                mainLabelY = moveLabelDown ? 10 : (subLabel.isEmpty() ? -18 : -24);
                subLabelY = moveLabelDown ? 26 : -20;
            }
