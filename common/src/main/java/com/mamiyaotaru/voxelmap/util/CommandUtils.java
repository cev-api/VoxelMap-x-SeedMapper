...
public final class CommandUtils {
    ...
    private static String getSharedCoordinates(Waypoint waypoint) {
        ...
        DimensionContainer dimension = waypoint.dimensions.first();
        return waypoint.getXInDimension(dimension) + ", " + waypoint.getY() + ", " + waypoint.getZInDimension(dimension);
    }
    ...
    private static Waypoint createWaypointFromXaero(Matcher matcher) {
        try {
            ...
            return new Waypoint(name, x, z, y, enabled, color[0], color[1], color[2], "", world, dimensions);
    ...
    private static Waypoint findWaypointAt(Waypoint waypoint) {
        for (Waypoint existingWaypoint : VoxelConstants.getVoxelMapInstance().getWaypointManager().getWaypoints()) {
            if (waypoint.getXInCurrentDimension() == existingWaypoint.getXInCurrentDimension() && waypoint.getZInCurrentDimension() == existingWaypoint.getZInCurrentDimension()) {
